...
import { askDocument, fetchDocument } from "../services/backendApi";
...
export default function AskQuestion() {
    const { id } = useParams();
    const [doc, setDoc] = useState(id ? null : { name: "General Assistant", id: "general" });
    ...
    useEffect(() => {
        ...
        async function loadDoc() {
            try {
                ...
                if (mounted) setPageError(error.message || "Failed to load document.");
            }
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const answer = id
                ? await askDocument(id, input)
                : "General assistant chat is available when you open a specific document.";
            const aiMsg = {
                id: Date.now() + 1,
                role: "ai",
                content: answer,
    ...
    if (id && pageError) {
        return <div className="p-6 text-red-400">{pageError}</div>;
    }
