...
import { fetchDocument, fetchDocumentSummary } from "../services/backendApi";
...
export default function DocumentDetail() {
            ...
            try {
                ...
                try {
                    ...
                    while (mounted && attempts < maxAttempts) {
                        const summaryResult = await fetchDocumentSummary(id);
                        const status = summaryResult?.status || "unknown";
                        ...
                        setSummary(summaryResult?.message || "No summary available.");
                    ...
                    setSummary(`Summary unavailable: ${summaryError.message || "Unknown error"}`);
                }
            } catch (err) {
                if (!mounted) return;
    ...
    const handleExportSummary = () => {
        ...
        docPDF.text(summary || "No summary available.", 10, 30, { maxWidth: 180 });
        docPDF.save(`${doc?.name}_summary.pdf`);
    };

    if (loading) {
        return <div className="text-slate-400 p-8">Loading document...</div>;
    }
        ...
        <div className="h-[calc(100vh-6rem)] md:h-[calc(100vh-5rem)] flex flex-col w-full max-w-[1600px] mx-auto animate-fade-in">
            ...
            <div className="flex-1 grid lg:grid-cols-12 gap-6 min-h-0">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <div className="p-3 bg-white/5 rounded-lg border border-white/5">
                                ...
                                <span className="text-emerald-400 text-sm font-medium flex items-center capitalize">
                                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 mr-2" /> {doc.status}
                                </span>
                            </div>
                            <div className="p-3 bg-white/5 rounded-lg border border-white/5">
                    ...
                    <div className="bg-[#181b21] border border-white/5 rounded-2xl flex-1 flex flex-col overflow-hidden relative">
                        ...
                        <div className="p-6 text-slate-300 space-y-4 text-sm leading-relaxed overflow-y-auto">
                            ...
                            {summaryStatus === "processing" && (
                                <p className="text-xs text-slate-400">Summary is still generating. Refreshing automatically...</p>
                            )}
                ...
                <div className="lg:col-span-5 h-full flex flex-col">
                    <ChatInterface docId={doc.id} docName={doc.name} selectedPersona={persona} />
                </div>
